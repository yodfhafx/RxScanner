...
//  rxScan
//
//  Created by Yodfha on 9/18/2559 BE.
...
class pickTableViewController: UITableViewController {
    ...
    func filterContentForSearchText(searchText: String, scope: String = "All") {
        filteredNames = episodes.filter { name in
            ...
            return name.title!.lowercaseString.containsString(searchText.lowercaseString) || name.author!.lowercaseString.containsString(searchText.lowercaseString)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "SendData" {
            if let indexPath = self.tableView.indexPathForSelectedRow {
                ...
                let controller = (segue.destinationViewController) as! TodoSchedulingViewController
...
extension pickTableViewController: UISearchResultsUpdating {
